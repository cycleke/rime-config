...
local function autocap_filter(input, env)
	...
	if
		codeLen == 1 -- 码长为 1
		or code:find("^[%l%p]") -- 输入码首位为小写字母或标点
	then -- 输入码不满足条件不判断候选项
	...
	for cand in input:iter() do
		...
		-- 不转换：
		if
			text:find("[^%w%p%s]") -- 候选词包含非字母和数字、非标点符号、非空格的字符
			or text:find("%s") -- 候选词中包含空格
			or pureText:find("^" .. code) -- 输入码完全匹配候选词
			or (
				cand.type ~= "completion" -- 单词与其对应的编码不一致
				and pureCode:lower() ~= pureText:lower()
			) -- 例如 PS - Photoshop
		then
			yield(cand)
