...
function mint_date_time_translator(input, seg)
	...
	if input == "date" then
		...
		yield(
			make_hight_quality_candidate("date", seg.start, seg._end, os.date("%Y 年 %m 月 %d 日"), "中文简写")
		)
		...
		yield(
			make_hight_quality_candidate(
				...
				seg.start,
				seg._end,
				date_y_chinese .. date_m_chinese .. date_d_chinese,
				"中文大写"
			)
		)
	...
	if input == "time" then
		...
		yield(make_hight_quality_candidate("time", seg.start, seg._end, os.date("%H 点 %M 分 %S 秒"), "中文"))
	end
	...
	if input == "week" then
		...
		yield(
			make_hight_quality_candidate(
				...
				seg.start,
				seg._end,
				"周" .. weekTab[tonumber(os.date("%w") + 1)],
				"星期"
			)
		)
		...
		yield(
			make_hight_quality_candidate(
				...
				seg.start,
				seg._end,
				"星期" .. weekTab[tonumber(os.date("%w") + 1)],
				"星期"
			)
		)
