...
local function number2cnChar(num, flag, digitUnit, wordFigure) --flag=0中文小写反之为大写
	...
	elseif lens < 13 then
		result = formatNum(string.sub(num, 1, -9), flag)
			.. digitUnit[2]
			.. formatNum(string.sub(num, -8, -5), flag)
			.. digitUnit[1]
			.. formatNum(string.sub(num, -4, -1), flag)
	else
		result = ""
	end
...
local function number_translatorFunc(num)
	...
	if numberPart.dot ~= "" then
		...
		table.insert(
			result,
			{ number2cnChar(numberPart.int, 0, { "万", "亿" }, { "〇", "一", "十", "" }), "〔数字小写〕" }
		)
		table.insert(
			result,
			{ number2cnChar(numberPart.int, 1, { "萬", "億" }, { "零", "壹", "拾", "" }), "〔数字大写〕" }
		)
	end
	table.insert(
		result,
		{
			number2cnChar(numberPart.int, 0) .. decimal_func(
				numberPart.dec,
				{ [1] = "角", [2] = "分", [3] = "厘", [4] = "毫" },
	...
	local number2cnCharDec = decimal_func(
		numberPart.dec,
		{ [1] = "角", [2] = "分", [3] = "厘", [4] = "毫" },
		{ [0] = "零", "壹", "贰", "叁", "肆", "伍", "陆", "柒", "捌", "玖" }
	)
	if
		string.len(numberPart.int) > 4
		and number2cnCharInt:find("^拾[壹贰叁肆伍陆柒捌玖]?")
		and number2cnCharInt:find("[万亿]")
	then -- 简易地规避 utf8 匹配问题
...
local function number_translator(input, seg, env)
	...
	env.number_keyword = env.number_keyword
		or env.engine.schema.config:get_string("recognizer/patterns/number"):sub(2, 2)
